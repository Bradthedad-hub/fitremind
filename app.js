...
// Theme/Font, Wiederholungen (Wochentage oder alle N Tage), CSV/ICS,
// YouTube, Kurz-Notiz, Ausführliche Beschreibung (ein-/ausklappbar),
// Install-Prompt, Notifications, Name/Icon anpassen, Service Worker
...
function applyUI(){
  ...
  if (ui.displayName){
    $("#app-title")?.insertAdjacentHTML("afterbegin", "");
    $("#app-title").innerHTML = `💪 ${ui.displayName} <span class="chip">PWA</span>`;
    document.title = ui.displayName + " – Übungen & Erinnerungen";
  }
  if (ui.iconDataUrl){
    const fav = $("#app-favicon"); if (fav) fav.href = ui.iconDataUrl;
    const prev = $("#iconPreview"); if (prev) prev.src = ui.iconDataUrl;
  }
}
applyUI();
...
try { if (tzSelect) tzSelect.value = Intl.DateTimeFormat().resolvedOptions().timeZone || "Europe/Berlin"; }
catch(e){ if (tzSelect) tzSelect.value = "Europe/Berlin"; }
...
function clearForm(){
  ...
  $("#yt").value=""; $("#notes").value=""; $("#longNotes")?.value="";
  setCheckedDays([]);
  if (startdate) startdate.valueAsDate = new Date();
...
function render(){
  ...
  data.forEach((it, idx)=>{
    ...
    const recur = (it.recurrence==="interval") ? `alle ${it.intervalDays||1} Tage`
                                               : ((it.days||[]).join(",") || "—");
...
function toICS(ex){
  ...
  const dt = new Date(ex.start || new Date());
